...
- 音乐：用户提供的 `Carpenters - Top Of The World.mp3`，原文件保留；网页副本 `public/audio/top-of-the-world.mp3`。
- 专辑封面：Carpenters《A Song for You》官方发布页 https://www.carpentersofficial.com/releases-archive/song-you/ 。原图 https://www.carpentersofficial.com/wp-content/uploads/sites/1757/2018/11/release_201811_39d0e2abfa208542a4c5cdb412aca21d45bc64c0.jpg 。本地文件 `public/images/a-song-for-you.jpg`。
- 插画使用内置 image_gen 生成，未使用 CLI 模式。最终文件：`public/images/hallway.png`、`public/images/door.png`、`public/images/envelope.png`。
...
- 三维枣模型：用户提供的 `hongzao_red_date.glb`，网页本地副本为 `public/models/hongzao-red-date.glb`。这是自包含的 GLB 2.0 模型，含果实、枣柄和顶点色，不需要外部纹理；网页复用同一组网格和材质组成三颗旋转红枣。
- 礼盒插画：通过内置 `image_gen` 工具生成，非 CLI 模式，最终文件 `public/images/jujube-gift.png`。透明背景，上半张是独立盒盖，下半张是装满红枣的盒身；代码裁切显示并移动盒盖，没有修改用户提供的海报。
